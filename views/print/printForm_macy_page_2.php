...
<div id="container1"> 
  <table>
    ...
    if ($num_mi > 0) {
      ...
      $SO_PO_QTY_TOTAL_2 = 0;
      while ($row_mi2 = mysqli_fetch_array($result_mi2) ) {
        
        //GET MI                      
        $MI_MATERIAL_CODE     = !empty($row_mi2['MI_MATERIAL_CODE'])?trim($row_mi2['MI_MATERIAL_CODE']):"";               
        if (substr($MI_MATERIAL_CODE,0,2) !== '6-' ) {
          continue;//chỉ lấy giá trị material code bắt đầu bằng '6-'
        }
          $MI_PO_NO             = !empty($row_mi2['MI_PO_NO'])?trim($row_mi2['MI_PO_NO']):"";
          $MI_PO_SO_LINE        = !empty($row_mi2['MI_PO_SO_LINE'])?trim($row_mi2['MI_PO_SO_LINE']):"";
          
          $MI_MATERIAL_DES      = !empty($row_mi2['MI_MATERIAL_DES'])?trim($row_mi2['MI_MATERIAL_DES']):"";
          $MI_MATERIAL_QTY      = !empty($row_mi2['MI_MATERIAL_QTY'])?trim($row_mi2['MI_MATERIAL_QTY']):0;
          $MI_MATERIAL_QTY      = (int)$MI_MATERIAL_QTY;
          $MI_MATERIAL_QTY_TOTAL += $MI_MATERIAL_QTY;
          
          $MI_INK_CODE          = !empty($row_mi2['MI_INK_CODE'])?trim($row_mi2['MI_INK_CODE']):"";
          $MI_INK_DES           = !empty($row_mi2['MI_INK_DES'])?trim($row_mi2['MI_INK_DES']):"";
          $MI_INK_QTY           = !empty($row_mi2['MI_INK_QTY'])?trim($row_mi2['MI_INK_QTY']):0;
          $MI_INK_QTY           = (int)$MI_INK_QTY;
          $MI_INK_QTY_TOTAL     += $MI_INK_QTY;
          
          //GET INTERNAL_ITEM, SO_QTY, FROM SOGRID
          $query_soline2 = "SELECT SO_INTERNAL_ITEM,SO_ORDER_ITEM,SO_PO_QTY,SO_WIDTH,SO_HEIGHT  FROM $table_so WHERE SO_PO_NO = '$MI_PO_NO' AND SO_LINE = '$MI_PO_SO_LINE' ";
          $result_soline2 = mysqli_query($conn, $query_soline2);
          if (!$result_soline2) {
              echo "[ERROR V]. Query sai (SO)";
              return false;
          }
          $num_soline = mysqli_num_rows($result_soline2);
          if ($num_soline > 0) {
              
              $row_so2 = mysqli_fetch_array($result_soline2);
              ////GET DATA FROM SO GRID
              $SO_INTERNAL_ITEM = !empty($row_so2['SO_INTERNAL_ITEM'])?trim($row_so2['SO_INTERNAL_ITEM']):"";
              $SO_ORDER_ITEM    = !empty($row_so2['SO_ORDER_ITEM'])?trim($row_so2['SO_ORDER_ITEM']):"";
              $SO_PO_QTY        = !empty($row_so2['SO_PO_QTY'])?trim($row_so2['SO_PO_QTY']):0;
              $SO_PO_QTY        = (int)$SO_PO_QTY;
              $SO_PO_QTY_TOTAL_2  += $SO_PO_QTY;

              $BARCODE_PO_QTY_2         = '<img style="height:120%" src="../../data/barcode.php?text='.$SO_PO_QTY_TOTAL_2.'" />';
              // $SO_WIDTH         = !empty($row_so2['SO_WIDTH'])?trim($row_so2['SO_WIDTH']):"";
              // $SO_HEIGHT        = !empty($row_so2['SO_HEIGHT'])?trim($row_so2['SO_HEIGHT']):"";
          }//if soline
          ...
          <tr>
            ...
            <td class="Col Col-8" style="font-size:15px;font-weight:normal;"><?php echo $MI_INK_DES; ?></td>
          </tr>
        <?php
    ...
    <tr>
      ...
      <td class="Col Col-4" style="font-weight:bold;color:red; font-size:16px; "><?php echo number_format($SO_PO_QTY_TOTAL_2); ?></td>
    ...
    <tr>
      <td class="" colspan=4 style="font-weight:bold;color:red;text-align:right;  " ><?php echo $BARCODE_PO_QTY_2; ?></td>
  ...
  <div class="case-label" style="text-align:center;" >CASE LABEL </div>
</div> 

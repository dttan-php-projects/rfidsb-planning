<?php	
	...
	if( $num > 0 ){ 
		...
		foreach ($rowsResult as $key => $formatData){
			...
			$INTERNAL_ITEM 		= !empty($formatData['INTERNAL_ITEM'])?($formatData['INTERNAL_ITEM']):'';
			$FORM_TYPE 			= !empty($formatData['FORM_TYPE'])?($formatData['FORM_TYPE']):'';
			$CREATED_BY 		= !empty($formatData['CREATED_BY'])?($formatData['CREATED_BY']):'';
			$CREATED_TIME 		= !empty($formatData['CREATED_TIME'])?($formatData['CREATED_TIME']):'';

			$data[]=[
				$ID,$INTERNAL_ITEM,$FORM_TYPE,$CREATED_BY,$CREATED_TIME
			];
		}//end foreach
	...
	*/
	...
	$excel->getActiveSheet()->setCellValue('B1', 'INTERNAL ITEM');
	$excel->getActiveSheet()->setCellValue('C1', 'FORM TYPE');
	$excel->getActiveSheet()->setCellValue('D1', 'CREATED BY');
	$excel->getActiveSheet()->setCellValue('E1', 'CREATED DATE');
	...
	$filename = "RFIDSB_Report_SettingForm_".date("d_m_Y__H_i_s");
